...
public class Eq14Command : ICommand
{
    private readonly string _arg1;
    private readonly string _arg2;

    public Eq14Command(string arg1, string arg2)
    {
        _arg1 = arg1;
        _arg2 = arg2;
    }
    ...
    public void Execute(ProgramState state)
    {
        ...
        if (count == 0)
        {
            if (state.Proof[A].IsAssertion || state.Proof[B].IsAssertion)
            {
                state.Proof[A].IsAssertion = true;
                state.Proof[B].IsAssertion = true;
            }
        }
